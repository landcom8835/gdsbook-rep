...
<!-- #region jp-MarkdownHeadingCollapsed=true user_expressions=[] -->
##### **椭球体模型**(ellipsiod)
...
```{figure} ./Resources/圆柱投影相割示意图.png
:alt: 圆柱投影相割示意图
:align: center
:width: 43%
```        
...
高斯-克吕格投影是按分带方法各自进行投影，各带独自构成高斯平面直角坐标系。以中央经线投影为纵轴(x)，赤道投影为横轴(y)，两轴交点即为各带的坐标原点。   
...
栅格数据的表达与像元的大小(即栅格的分辨率)有关：分辨率越高(即像元越小)表达的地理实体、地理现象越精细，表达的误差越小，表达的数据量也越大；分辨率越低(即像元越大)表达的地理实体、地理现象越粗糙，表达的误差越大，表达的数据量越小。   
...
Python的以上特性使之成为目前最流行的编程语言之一，越来越多的开发人员转向Python编程，越来越多的第三方提供各种各样功能丰富的Python包。Python已成为高校计算机程序设计入门课教学中广泛使用的语言。Python的流行也意味着有越来越多资源可以学习使用。
<!-- #endregion -->
...
**Anaconda**通过管理Python包、开发环境、Python版本，大大简化了Python开发的工作流程。不仅可以方便地安装、更新、卸载Python包，而且安装时能**自动安装相应的依赖包**，同时还能**使用不同的虚拟环境隔离不同要求的项目**。

**conda**在命令行界面上工作，比如Windows上的Anaconda Prompt和macOS和Linux上的终端。 

**Anaconda Navigator**是一个桌面图形用户界面，允许用户启动应用程序并轻松管理conda包(packages)、环境(environments)和通道(channels)，而无需使用命令行命令。   

Anaconda集成工具可从Anaconda官网（https://www.anaconda.com/ ）直接下载并安装，本课程选择的是Windows下的64-Bit Graphical Installer.  

<!-- #endregion -->

<!-- #region slideshow={"slide_type": "slide"} user_expressions=[] -->
Anaconda安装完成后，可以从Windows“开始”->"Anaconda3(64-bit)"程序组中看到Anaconda集成工具包括：
...
<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
##### **Anaconda Navigator**

**Anaconda Navigtor**用于管理工具包和环境的图形用户界面，可以启动应用程序并轻松管理conda程序包(packages)、环境(environments)和通道(channels)，而无需使用命令行命令。    

启动Anaconda Navigator有以下两种方法：     
1. 鼠标点击Anaconda Navigator菜单栏启动 

2. 打开"Anaconda Prompt"并键入命令anaconda-navigator

anaconda-navigator界面如图1-15所示。
```{figure} ./Resources/Navigator界面.png
:alt: Navigator界面
:align: center
:width: 100%
```                
<center>图1-15 Navigator界面</center>   

<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
启动Anaconda Navigator后，通过Navigator的**Home属性页**可以安装、启动自带的应用程序，如Jupyter Lab、Jupyter Notebook、VS Code、QT Console、RStudio、Spyder等。   

通过Navigator的**Environments属性页**可以创建（Create）、克隆（Clone）、导入（Import）、删除（Remove）、切换Python环境（Environments）。Navigator可以从 Anaconda Cloud或者本地Anaconda仓库搜索所需要的packages（**若查找不到，则需要使用conda命令行安装**），同时，**在相应的环境中可以添加、更新所需要的包**。   

通过Navigator的**Learning属性页**可以查看相关主题的文献资料、文档等。   

通过Navigator的**Community属性页**可以了解相关社团的会议、博客等信息。      
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
##### **conda**

**conda**是**Anaconda Prompt**命令行环境下的**包管理器**和**环境管理器**。conda 是一个Python项目，但它支持许多语言，如：Python、 R、 Ruby、Lua、Scala、Java、JavaScript、 C/ C++、 FORTRAN等。 
...
从Windows的“开始”-> "Anaconda3(64-bit)"程序组中点击"Anaconda Prompt"启动conda(可以通过鼠标右键点击“以管理员身份运行”)，如图1-16所示。
```{figure} ./Resources/conda命令行界面.png
:alt: conda命令行界面
:align: center
:width: 60%
```       
<center>图1-16 conda命令行界面</center>   
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
(2) **管理conda**

   * **查看conda版本**（同时核查有没有安装conda）,在"Anaconda Prompt"命令行窗口当前提示符下输入以下命令：    
conda --version    
   * **更新conda**    
conda update conda    
   * **查看conda帮助信息**   
...
`<env_name>`为要创建的环境名。建议以英文命名，且不加空格，名称两边不加尖括号“<>”。   
...
conda update --all  `更新所有包`    
  以上的update也可使用upgrade。

**另外，还可以使用pip包管理工具管理Python包。**
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
#### **Jupyter Lab与Jupyter Notebook**    

Jupyter Lab与Jupyter Notebook是基于Web的Python交互式开发环境，在一个Notebook文档中可以同时包含**说明文本、公式、图片、程序代码及代码运行结果等**内容，是开放科学、开放研究的重要工具，**仅需文件共享**，他人就能很方便地重现你的研究结果。      
**Jupyter Lab**包含了**Jupyter Notebook**所有功能，并增加了较多功能。**Jupyter Lab**最大的更新是模块化的界面，可以在同一个窗口以标签的形式同时打开多个文档，同时增加了功能强大的扩展模块，另外还具有命令行(Console)、终端(Terminal，如Windows PowerShell)等功能。    
安装Anaconda会自动安装**Jupyter Lab**和**Jupyter Notebook**，可以从**Anaconda Navigtor**中启动**Jupyter Lab**和**Jupyter Notebook**。另外还可以通过Anaconda Prompt命令行安装、启动**Jupyter Lab**和**Jupyter Notebook**。

安装**Jupyter Notebook**的conda命令: conda install jupyter notebook    
安装**Jupyter Lab**的conda命令:  conda install jupyter lab   
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
Anaconda Prompt命令行中，可以从指定目录启动**Jupyter Lab**和**Jupyter Notebook**：
* 在Anaconda Prompt命令行中，CD到指定目录，如d: -> cd \gispython   
*  激活Python工作环境，如：conda activate gds2022
* 启动Jupyter Lab或jupyter notebook，即在Anaconda Prompt命令行中直接输入：jupyter lab或jupyter notebook.   

启动后，默认的服务器的运行地址是 http://localhost:8888 (每新增一个服务，端口号+1，即8889、8890......)。 只要服务器仍在运行，随时可以在浏览器中输入 http://localhost:8888 返回到 web 页面中。
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
**JupyterLab扩展**

JupyterLab扩展包含安装到JupyterLab并在浏览器中运行的JavaScript。扩展包含一个或多个扩展JupyterLab的插件。JupyterLab扩展有两种类型：
* a **source extension**(源码扩展) ：需要在安装时重新构建(rebuild)JupyterLab，重新构建JupyterLab需要安装Node.js.
* a **prebuilt extension**(预编扩展)：不需要重新构建JupyterLab.    
首选的JupyterLab扩展类型是预编扩展，因为它不需要重新构建JupyterLab JavaScript文件。   
Installing Node.js
...
<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
通过JupyterLab Extension Manager管理扩展

可以使用JupyterLab中的扩展管理器来管理扩展，管理界面如图1-17左侧所示，可以安装、禁用相关扩展。出于安全原因，该功能可能已被系统管理员禁用，要安装扩展，先要使“The JupyterLab development team is excited to have a robust third-party extension community. However, we do not review third-party extensions, and some extensions may introduce security risks or contain malicious code that runs on your machine.”有效。    
```{figure} ./Resources/JupyterLab扩展管理器.png
:alt: JupyterLab扩展管理器
:align: center
:width: 85%
```       
<center>图1-17 JupyterLab扩展管理器</center>   
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
**Jupyter Notebook文档基本结构**

Jupyter Notebook文档由基本的单元格组成，单元格有代码单元格(Code)、Markdown单元格两种基本类型，新建单元格的默认类型为代码格式。每个单元格在编辑状态下可以任意的输入代码和注释说明(Markdown)。单元格绿色代表内容可编辑状态（比如输入代码），蓝色代表单元格可操作状态（比如删除单元格，必须回到蓝色），而蓝色与绿色之间可以用Esc 和 Enter 来切换。   
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
 Jupyter Notebook文档基本结构如图1-18所示。   
```{figure} ./Resources/Notebook文档基本结构.png
:alt: Notebook文档基本结构
:align: center
:width: 90%
```       
<center>  图1-18 Jupyter Notebook文档基本结构。 </center>   
<!-- #endregion -->

<!-- #region user_expressions=[] -->
#### ***Markdown与MyST Markdown基本语法***

Markdown是一种轻量级标记语言，它允许人们使用易读易写的纯文本格式编写文档。    
Markdown 编写的文档可以导出HTML 、Word、图像、PDF、Epub等多种格式的文档。    
Markdown 编写的文档后缀为 .md, .markdown。   
Markdown 能被使用来撰写电子书，如：Gitbook。当前许多网站都广泛使用 Markdown 来撰写帮助文档或是用于论坛上发表消息。     
    
[MyST](https://myst-tools.org/)(Markedly Structured Text，显著结构化的文本)专为 Sphinx 项目使用而设计的 Markdown风格。它是CommonMark Markdown和一些额外的语法片段的组合，以支持 Sphinx的功能，可以用纯Markdown编写Sphinx文档。MyST是[Jupyter Book](https://jupyterbook.org/en/stable/intro.html)使用的核心技术之一。MyST语法可以在Jupyter notebook中的markdown文件或markdown单元格中使用。
<!-- #endregion -->

<!-- #region user_expressions=[] -->
##### **MyST Markdown概述**   

与普通的Markdown相比，MyST Markdown增加了一些特有功能，如指令(directive)、角色(role)、交叉引用(cross-references)、外部引用(external references)、引用(citations)、内联数学公式(inline math)、悬停链接(Hover link)等。
    
MyST还支持在公共服务(如Wikipedia或DOI链接)中链接到其他文档的丰富信息，这些信息支持丰富的链接预览以及包含引用的简单方法。  

**角色**和**指令**是Jupyter Book中最强大的两个工具。它们有点像函数，但是用标记语言编写。它们都有类似的功能，但是**角色是在一行内编写的**，而**指令则可跨越多行**。它们都接受不同类型的输入，如何处理这些输入取决于所使用的特定角色或指令。   

**角色**的一般格式：\{rolename\}\`role content\`
<!-- #endregion -->

<!-- #region user_expressions=[] -->
**指令**是包含标识符(identifier)、参数(arguments)、选项(options)和内容(content)的多行容器，如警告(admonitions)、图(figures)和方程式(equations)。指令使用3个反引号(` ``` `)或冒号(`:::`)包围，使用花括号\{directivename\}命名，如：   

\```{somedirectivename}   
Some directive content example   
\```   

这只会在一个名为somedirectivename的指令已经存在(但命为somedirectivename的指令确实不存在)的情况下起作用。有许多预定义的指令与Jupyter Book相关联，如{note}、{warning}、{admonition}、{important}等。例如，要在内容中插入一个注释框(note box)，可以使用以下指令：   
...
<!-- #region user_expressions=[] -->
指令可以嵌套，外围指令需增加一个符号(\:或\`)，如   
\::::{important}   
\:::{note}   
This text is `**standard**` \_Markdown\_   
\:::   
\::::   
...
1. This is a list item1
2. This is a list item2
3. This is a list item3
4. This is a list item4
<!-- #endregion -->
...
可以使用GitHub风格的Markdown来创建任务列表(Task Lists)，这些列表可能是只读的或可编辑的，这取决于用例或使用的主题。

MyST Markdown任务列表语法在普通的列表符号(*、-、+)后跟[X]或[ ]，[X]表示被选，如：   
\- \[X\] 求交叠置分析x   
\- \[ \] 求并叠置分析   
结果为：
- [X] 求交叠置分析
...
定义列表(Definition lists)基于Pandoc的定义列表规范，其语法格式为每个名称术语的下一行由冒号或波浪号开始，即：  
...
每个术语必须在一行中，这一行后面可以有一个空行，并且后面必须有一个或多个定义。定义以冒号或波浪号开头，可以缩进一个或两个空格。一个术语可以有多个定义，每个定义可以由一个或多个块元素(段落、代码块、列表等)组成。
<!-- #endregion -->
...
<!-- #region user_expressions=[] -->
MyST删除线使用del或strike角色, 如\{del\}\`这是加删除线的文字\`结果显示为  
{del}`这是加删除线的文字`    
<!-- #endregion -->
...
Markdown**下划线**(underline)可以使用HTML的u标签，即：\<u\>这是加下划线的文本\<\/u\>   
MyST Markdown使用u或underline角色，如\{u\}\`这是加下划线的文本\`结果为：{u}`这是加下划线的文本`        
<!-- #endregion -->
...
使用HTML中上下标的语法:    
上标：`a<sup>上标</sup>`  a<sup>上标</sup>      
下标：`a<sub>下标</sub>`  a<sub>下标</sub>     
sup指的是superscript上标，sub指的是subscript下标    
HTML上下标示例：    

`H<sub>2</sub>O`=H<sub>2</sub>O , `CO<sub>2`=CO<sub>2</sub>      
`QGIS<sup>TM</sup>`=QGIS<sup>TM</sup>
    
MyST Markdown使用sub、sup角色分别表示下标、上标，如：  
H\{sub\}`2`O, 4\{sup\}`th` of July   
结果为：   
H{sub}`2`O, 4{sup}`th` of July
<!-- #endregion -->
...
MyST Markdown使用abbr角色来表示缩略词(Abbreviation)，其语法如下：   
\{abbr\}\`缩略词\(缩略词表示的文本\)\`，当鼠标悬停于缩略词时，会弹出缩略词表示的文本，如：   
Well \{abbr\}\`MyST\(Markedly Structured Text\)\` is cool!   
显示为：Well {abbr}`MyST (Markedly Structured Text)` is cool!
<!-- #endregion -->

<!-- #region user_expressions=[] -->
##### **脚注**

脚注(Footnotes)使用pandoc规范。脚注用[^label]标记，label可以是任何字母数字字符串(不含空格)，不区分大小写。这将创建一个指向脚注定义的链接，该定义是以相同的[^label]开头，然后是脚注的文本。如果label是一个整数，那么它将始终使用该整数来呈现标签(即它们是手动编号的)。对于任何其他标签，它们将按照被引用的顺序自动编号，跳过任何手动编号的标签。如：   
...
可以在一行中用三个以上的星号*、减号-、下划线_来建立一个分隔线，行内不能有其他内容，也可以在星号或减号间插入空格。
...
`[江西理工大学](https://www.jxust.edu.cn/)`  [江西理工大学](https://www.jxust.edu.cn/)
...
* **使用标准的Markdown语法**        
插入图片最简单的方法是使用标准的Markdown语法：

```![图片替代文字](图片地址 "可选的图片提示")```
    
说明：

1. 图片替代文字在图片无法正常显示时使用，正常情况下可以为空。
2. 图片地址可以是本地图片的路径也可以是网络图片的地址。
3. 本地图片支持相对路径和绝对路径两种方式。
4. "可选的图片提示"，当鼠标移到图片上时显示的内容。
5. 这种语法不能设置图片的长宽、对齐方式等属性。    
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
插入图片示例： 

`![GIS应用开发(第二版)](./Resources/earth.png)`   
![GIS应用开发(第二版)](./Resources/earth.png '可选的图片提示')   
图1-19 插入图片-Markdown示例
<!-- #endregion -->

<!-- #region slideshow={"slide_type": "subslide"} user_expressions=[] -->
* **使用HTML的img标签**   

可以通过HTML的img标签插入图片，利用width、height属性设置图片的宽度、高度像素大小或百分比。
`<img src="./Resources/earth.png" width="400">`   
<img src="./Resources/earth.png" width="400">   
图1-20 插入图片-img示例
<!-- #endregion -->
...
MyST Mrakdown提供了image、figure两个指令，用于插入图片。image指令允许指定图片的长宽、对齐方式，还可以添加其他类。figure指令还可以包含图片标题，并允许在文档的其他部分交叉引用此标题。   
\```{image} ./Resources/earth.png   
`:alt: imgexample`   
`:align: center`   
`:width: 40%`   
\```    
...
:width: 40%
```
<center> 图1-21 插入图片-image指令示例 </center>   

\```\{figure\} ./Resources/figure示例.jpg   
`:name: myFigure`   
`:alt: figure示例`   
`:align: center`   
`:width: 600`      
...
:width: 600
```   
<center> 图1-22 插入图片-figure指令示例 </center>   
...
Markdown两种输入公式的方法：   
   * 一是行内公式，用一对美元符号\$包围。   
   * 二是跨行公式，用一对紧挨的两个美元符号\$\$包围。   

MyST Markdown为公式提供了math角色(行内)和指令(跨行)。math角色如：   
...
math指令中可选的label参数可用于文档的其它位置交叉引用。   
\$\$跨行公式示例：       
$$
\label{maxwell}
\begin{aligned}
...
\sum用来表示求和符号，其下标表示求和下限，上标表示上限。如```$\sum_1^nx_i$``` = $\sum_1^nx_i$；```$\sum_{i=0}^n x_i$``` = $\sum_{i=0}^n x_i$

\int用来表示积分符号，其上下标表示积分的上下限。如```$\int_1^\infty f(x)$``` = $\int_1^\infty f(x)$
...
**根式**使用```$\sqrt[a]b$```即$\sqrt[a]b$来表示，其中，方括号内的值用来表示开几次方，省略方括号则表示开方。
...
**Spyder**是用Python编写的免费开源Python集成开发环境，由科学家、工程师和数据分析人员设计。**Spyder**是主流Python集成开发环境（IDE）之一，它综合了开发工具的高级编辑、分析、调试和分析功能，以及科学软件包的数据探索、交互式执行、深度检查和可视化功能。

Spyder的notebook插件，用于在Spyder中使用Jupyter notebook。目前，它支持创建新notebook、打开文件系统中的任何notebook以及在任何位置保存notebook等基本功能。

还可以使用Spyder的文件切换器在notebook之间轻松切换，并打开连接到notebook内核的IPython控制台，在变量资源管理器中检查其变量。
<!-- #endregion -->
...
在Python语言中，所有标识符可以包括英文、数字以及下划线，但要符合以下规则：
   ...
   (3). 标识符区分大小写的。   
   (4). 标识符不能使用关键字。   
   (5). 标识符不能和 Python 中内置函数和内置对象相同。       
在Python环境下，可以通过`dir(__builtins__)`查看所有内置函数和内置对象。
<!-- #endregion -->
...
以**单下划线开头**(_xxx)的标识符代表不能直接访问的类属性，需要通过类提供的接口进行访问，不能用“from xxx import x” 导入。     
以**双下划线开头**(__xxx)的标识符代表类的私有成员。   
...
在Python中，关键字作为语言的保留标识符，不能用于其它用途，否则会引起语法错误。可以导入keyword模块关键字查看所有Python关键字。
<!-- #endregion -->
...
变量用于表示可变数据，通过变量名访问，不同变量通过变量名相互区分。Python变量名的命名规则：

   * 变量名只能包含字母、数字和下划线，但不能用数字开头。
...
Python语言中提供了几种数据类型，如数值（int、float、complex）、布尔（bool）、字符串（str）、列表（list）、元组（tuple）、字典（dict）、集合（set）等。   
* **整数型int**   
整数是没有小数部分的数值，分为正整数、0和负整数。例如下列值都是合法的整数：100 、 0、 -100   
* **浮点型float**   
浮点数是包含小数点的数或科学计数法表示的数。例如下列值都是浮点数：-56 、 0.19 、 10.0 、 5.12e2 、 3.14e-1 、22.1E-10   
* **复数型complex**   
复数由实数部分和虚数部分构成，复数形式为：实部+虚部 j 或者complex(a,b)3.14j 、 5.213e-11j 、 -0.987+0j   
<!-- #endregion -->

<!-- #region user_expressions=[] -->
* **布尔型**   
布尔型是一种表示逻辑值的简单类型，它的值只能是真或假中的一个，即True和Flase.   
<!-- #endregion -->
...
<!-- #region user_expressions=[] -->
* **字符串str**   
可以使用单引号、双引号、三引号来创建字符串。'Hello'、 "Python"、 '''Jupyter'''。
  字符串的基本操作：   
...
<!-- #region user_expressions=[] -->
* **列表list**
列表用方括号“[”和“]”将列表中的元素括起来，元素间用逗号分隔。[1,2,3] 、 ['apple','pear','orange']   
列表是 Python 序列的一种，可以使用索引（Index）访问列表中的某个元素（得到的是一个元素的值），也可以使用切片访问列表中的一组元素（得到的是一个新的子列表）。   
   * 使用索引访问列表元素的格式为：*listname[i]*   
其中，listname 表示列表名字，i 表示索引值。列表的索引可以是正数（表示从前往后的顺序），也可以是负数（表示从后往前的顺序）。   
   * 使用切片访问列表元素的格式为：*listname[start : end : step]*   
其中，listname表示列表名字，start表示起始索引，end表示结束索引，step 表示步长。
   * 添加列表元素    
使用append()方法在列表的末尾追加元素，该方法的语法格式如下：*listname.append(obj)*   
其中，listname 表示要添加元素的列表；obj 表示到添加到列表末尾的数据，它可以是单个元素，也可以是列表、元组等。   
使用 insert() 方法在列表中间某个位置插入元素，语法格式如下：listname.insert(index , obj)   
...
<!-- #region user_expressions=[] -->
* **元组tuple**   
元组用“(”和“)”作为边界将元素括起来，元素间用逗号分隔。从存储内容上看，元组可以存储整数、实数、字符串、列表、元组等任何类型的数据，并且在同一个元组中，元素的类型可以不同。如：(1,2,3)、 ('apple','pear','orange')、(1,'a',3)、("github.com",1, [2,'a'],("abc",3.0))   
    和列表类似，元组也是由一系列按特定顺序排序的元素组成。元组和列表（list）的不同之处在于：
...
<!-- #region user_expressions=[] -->
* **字典dict**   
Python 字典（dict）是一种无序的、可变的序列，它的元素以“键值对（key-value）”的形式存储。字典类型是 Python 中唯一的映射类型，是键值对的映射。字典中每个元素都包含键和值两部分。字典用大括号{ }来表示，键和值间用冒号分隔，元素之间用逗号分隔。如：**scores = {'语文':122,'数学':110,'英语':125}**，访问字典元素：**scores['语文']**    
    字典的主要特征：
...
<!-- #region user_expressions=[] -->
字典的数据类型为 dict，可使用 dir(dict) 来查看该类型包含哪些方法，例如：
<!-- #endregion -->
...
<!-- #region user_expressions=[] -->
dict主要方法有：['clear', 'copy', 'fromkeys', 'get', 'items', 'keys', 'pop', 'popitem', 'setdefault', 'update', 'values']。
   * keys() 方法用于返回字典中的所有键（key）；
   * values() 方法用于返回字典中所有键对应的值（value）；
   * items() 用于返回字典中所有的键值对（key-value）；
   * copy() 方法返回一个字典的拷贝，也即返回一个具有相同键值对的新字典；
   * update() 方法可以使用一个字典所包含的键值对来更新己有的字典；
   * pop() 和 popitem() 都用来删除字典中的键值对，不同的是，pop(key) 用来删除指定的键值对，而 popitem() 用来随机删除一个键值对。
...
<!-- #region user_expressions=[] -->
* **集合set**   
集合由各种不可变类型元素组成，但元素之间没有任何顺序，并且元素不能重复。Python 集合将所有元素放在一对大括号 {} 中，相邻元素之间用“,”分隔，如：{element1,element2,...,elementn}，sa = {1,'c',(1,2,3)}
    由于集合中的元素是无序的，因此无法象列表那样使用索引访问元素。Python中，访问集合元素最常用的方法是使用循环结构，将集合中的数据逐一读取出来。如：   
    for ele in sa:
        print(ele)
...
<!-- #region user_expressions=[] -->
**set主要方法有**：['add', 'clear', 'copy', 'difference', 'difference_update', 'discard', 'intersection', 'intersection_update', 'isdisjoint', 'issubset', 'issuperset', 'pop', 'remove', 'symmetric_difference', 'symmetric_difference_update', 'union', 'update']   
    
   如：setname.add(element)、setname.remove(element)   
...
常用运算符有：算术运算符、比较运算符、测试运算符、逻辑运算符、位运算符。
* **算术运算符**   
用于实现数学运算功能。常用运算符功能如下：   
加+：实现两个对象相加   
减-：得到负数或表示一个数减去另一个数   
乘*：实现两个数相乘，或一个字符串重复n次   
除/：实现一个数除以另一个数   
取模%：返回除法的余数   
取整除//：返回商的整数部分，不包含余数   
幂**：返回x的y次幂   
...
<!-- #region user_expressions=[] -->
* **比较运算符**   
也称为关系运算符，表示两个变量或常量之间的关系，可用来比较两个数字的大小。常用比较运算符功能如下：     
大于>：如x>y，用于返回x是否大于y       
小于<：如x<y，用于返回x是否小于y    
大于等于>=：如x>=y，用于返回x是否大于等于y    
小于等于<=：如x<=y，用于返回x是否小于等于y    
等于==：用于比较对象是否相等   
不等于!=：用于比较对象是否不相等   

* **测试运算符**   
也称为成员运算符，测试实例中包含的一系列成员，包括字符串、列表或元组。常用测试运算符功能如下：     
in：如果在指定序列中找到值则返回True;否则，返回False        
not in：如果在指定序列中没有找到值则返回True;否则，返回False    
    
<!-- #endregion -->

<!-- #region user_expressions=[] -->
* **逻辑运算符**   
对两个变量或对象进行逻辑运算，结果返回True或False。常用逻辑运算符功能如下：     
逻辑与and：a and b，当 a 和 b 两个表达式都为真时，a and b 的结果才为真，否则为假。   
逻辑或or：a or b，当 a 和 b 两个表达式都为假时，a or b 的结果才是假，否则为真。   
逻辑非not：not a，如果 a 为真，那么 not a 的结果为假；如果 a 为假，那么 not a 的结果为真。相当于对 a 取反。   
<!-- #endregion -->

<!-- #region user_expressions=[] -->
* **位运算符**   
按位运算符是把数字看作二进制来进行计算的。按位运算法则如下（设变量 a 为 60，b 为 13）：   
按位与&：参与运算的两个值，如果两个相应位都为1，则该位的结果为1，否则为0。(a & b) 输出结果12，即二进制：0000 1100   
按位或|：只要对应的二个二进位有一个为1时，结果位就为1。(a | b) 输出结果61，即二进制：0011 1101   
按位异或^：当两对应的二进位相异时，结果为1。(a ^ b) 输出结果49，即二进制：0011 0001   
按位取反```~```：对数据的每个二进制位取反，即把1变为0，把0变为1。(~a ) 输出结果的二进制为：1100 0011   
左移位<<：运算数的各二进位全部左移若干位，<< 右边的数字指定了移动的位数，高位丢弃，低位补0。	a << 2 输出结果240，即二进制：1111 0000   
右移位>>：运算数的各二进位全部右移若干位，>>右边的数字指定了移动的位数。a >> 2 输出结果15，即二进制：0000 1111   
<!-- #endregion -->
...
* 顺序结构：程序按照从头到尾的顺序依次执行每一条 Python 代码，不重复执行任何代码，也不跳过任何代码。
* 选择结构（分支结构）：依据条件，有选择性的执行代码；即条件满足时，执行相应的程序代码，否则，执行其它代码。
* 循环结构：条件满足时，不断地重复执行同一段代码，直到条件不满足为止。   
<br>这里主要介绍选择结构和循环结构。
<!-- #endregion -->
...
在 Python 中，可以使用 if else 语句对条件进行判断，然后根据不同的结果执行不同的代码。Python 中的 if else 语句可以细分为三种形式，分别是 if 语句、if else 语句和 if elif else 语句，它们的语法:
...
`  `代码块n     
 ...
 注意：“代码块”由具由相同缩进量的若干条语句组成；if、elif、else 语句的最后都有冒号:
     
 在if else条件语句中，可以使用**pass空语句**，表示程序执行跳过此处。  
...
在执行 while 循环或者 for 循环时，只要循环条件满足，程序将会一直执行循环体。但在某些场景，我们可能希望在循环结束前就强制结束循环，Python 提供了 2 种强制离开当前循环体的方法：
   * break语句    
...
除了可以直接使用内置函数外，Python还支持自定义函数，即将一段有规律的、可重复使用的代码定义成函数，从而达到一次编写、多次调用的目的。   
* **Python函数定义**    
定义函数使用**def**关键字，具体的语法格式如下：    
...
其中，用```[]```括起来的为可选择部分，即可以使用，也可以省略。  
    注意：函数体代码须缩进。   
    函数定义中各部分的含义如下：   
   * 函数名：符合Python语法的标识符，函数名最好能够体现出该函数的功能。   
   * 参数列表：函数可以接收多少个参数，参数之间用逗号（,）分隔。函数也可以没有参数，这时一对空的“()”还必须保留。   
   * ```[return [返回值] ]```：可选部分，用于设置函数的返回值。一个函数，可以有返回值，也可以没有返回值，视实际情况而定。   
...
<!-- #region user_expressions=[] -->
使用geopy包计算球面距离、椭球面距离

   geopy.distance模块提供了计算两点之间球面距离、椭球面距离的方法：   
   distance.great_circle(): 使用haversine公式计算球面距离。   
   distance.geodesic(): 使用vincenty公式计算椭球面距离。    
<!-- #endregion -->
...
<!-- #region user_expressions=[] -->
* **函数参数的值传递和引用传递**    
在使用函数时，经常会用到形式参数和实际参数。形式参数是在定义函数时，函数名后面括号中的参数；实际参数是在调用函数时，函数名后面括号中的参数。Python 中，根据实际参数的类型不同，函数参数的传递方式可分为 2 种，分别为值传递和引用（地址）传递：
...
<!-- #region user_expressions=[] -->
* **Python函数参数--必要参数、关键字参数、默认参数、不定长参数**    
    
    **必要参数（位置参数、必备参数）**：即函数调用时必须要传的参数，传入函数的实际参数必须与定义函数时的形式参数的数量和位置对应，否则 Python 解释器会抛出TypeError异常，并提示缺少必要的参数。  
    **关键字参数**：指使用形式参数的名字来确定输入的参数值。通过此方式指定函数实参时，不再需要与形参的位置完全一致，只要将参数名写正确即可。   
    **默认参数**：在调用函数时如果不指定某个参数，Python 解释器会抛出异常。为了解决这个问题，Python 允许为参数设置默认值，即在定义函数时，直接给形式参数指定一个默认值。这样的话，即便调用函数时没有给拥有默认值的形参传递参数，该参数可以直接使用定义函数时设置的默认值。   
    Python定义**带有默认值参数的函数**，其语法格式如下：   
    def 函数名(...，形参名，**形参名=默认值**)：   
```        ```代码块         
   **不定长参数（可变参数）**：传入的参数个数是可变的。Python提供了两种方式来定义可变参数：           
    
   * 在参数前边加“*”，使用元组tuple来接受接收多个可变参数。   
   * 在参数前边加“**”，使用字典dict来接收任意多个以关键字参数赋值的实际参数。   
     
   **注意**：   
   * 在函数调用时，既可以根据位置参数来调用，也可以使用关键字参数来调用。在使用关键字参数调用时，可以任意调换参数传参的位置。   
   * 还可以使用位置参数和关键字参数混合传参的方式。但混合传参时，关键字参数必须位于所有的位置参数之后。   
   * 元组和字典参数的位置位于最后，且字典参数在元组参数后面，非元组与字典参数中，非关键字参数一定要排在关键字参数前面。   
...
* **Python函数式编程**
   ...
   一个函数可以接收另一个函数作为参数，这种函数称为高阶函数。函数式编程就是指这种高度抽象的编程范式，纯粹的函数式编程语言编写的函数没有变量，因此，任意一个函数，只要输入是确定的，输出就是确定的，这种纯函数称之为没有副作用。而允许使用变量的程序设计语言，由于函数内部的变量状态不确定，同样的输入，可能得到不同的输出，因此，这种函数是有副作用的。   
...
和其它编程语言一样，Python 也提供了操作文件（I/O）功能，比如打开文件、读取和追加数据、插入和删除数据、关闭文件、删除文件等。还提供了较多模块，例如fileinput模块、pathlib模块等，通过引入这些模块，可以获得大量文件操作的函数和方法（类属性和类方法），大大提高编写代码的效率。
...
在Windows上，文件路径使用反斜杠`"\"`作为文件夹之间的分隔符；在 OS X 和 Linux 上，使用正斜杠 "/" 作为路径分隔符。本课程使用的操作系统为Windows 10，在Python中，反斜杠`"\"`为转义字符，比如\n表示换行，\t表示制表符，路径中的`"\"`需要用`"\\"`表示，如：`C:\\Users\\Administrator`；此外，也可在常规路径前加r表示，如：`r'C:\Users\Administrator'`即为`C:\\Users\\Administrator`   

文件路径有 2 种表示方式：   
   * **绝对路径**：总是从根文件夹开始，Window 系统中以盘符（`C:\、D:\`）作为根文件夹，如：`d:\gisdata\geopy`。   
   * **相对路径**：文件相对于当前工作目录所在的位置。例如，当前工作目录为`d:\gisdata\geopy`，若文件demo.txt就位于这个geopy文件夹下，则demo.txt的相对路径表示为`.\demo.txt`（其中 .\ 就表示当前所在目录）。另外，..\ 表示当前所在目录的父目录。   
...
<!-- #region user_expressions=[] -->
   * **读取文件中的数据**        
    打开文件之后，就可以读取文件中的数据，Python提供了三个函数：       
       * **file.read()**函数：逐个字节或者字符读取文件中的内容；   
       * **file.readline()**函数：逐行读取文件中的内容；   
       * **file.readlines()**函数：一次性读取文件中多行内容。   
...
<!-- #region user_expressions=[] -->
   * **使用with上下文管理器**    
    open()打开文件使用之后，关闭文件（close()）非常重要，但时常会忘记关闭文件。Python提供with上下文管理器可以有效防止读取文件后忘记关闭文件的情况发生。   
        with open(file="./data/addresses.txt",mode="rt",encoding="utf-8") as f1:   
            data1 = f1.read()   
            print (data1)
        当操作完成后自动使用close()方法关闭打开的文件。
   
<!-- #endregion -->

```python
file = open("./data/addresses.txt",mode="rt",encoding="utf-8")
print(file)
```
...
在Python中，一个Python文件(以 .py 结尾)就称之为一个模块（Module）。模块包含了 Python 对象定义和Python语句。   

在开发过程中我们不会把所有的代码都写在一个.py文件 中。随着代码量的增大，可以按照功能将函数或者类分开存放到不同的.py文件中。这样代码更方便管理以及后期维护，也方便他人使用。
在开发过程中，我们也经常引用其他模块，例如：time、os、re 等等。
在Python中模块一般有如下3种：
   * Python内置模块
   * 第三方模块
   * 自定义模块
...
导入模块后，就可以通过module.variable或module.function()直接调用相关功能。   
<!-- #endregion -->

<!-- #region user_expressions=[] -->
#### **包（package）**   

Python中的包（Package）就是包含模块文件的目录，目录名称就是包名称，目录中可以包含目录，子目录也是包，但**包名称应该包含上一级目录的名称**。每一个包目录下，都有一个` __init__.py` 文件，若这个文件不存在，那么这个目录只是一个普通目录而不是一个包。`__init__.py` 文件中列出对外界提供的模块列表。
Python引入了按目录来组织模块是为了避免模块名冲突，不同包中的模块名可以相同。

**导入包**   
导入包的本质：导入一个包就是执行包下的`__init__.py`文件。   
导入包的语法与导入模块类似，要**注意包、子包、模块等的层次**，如：   
import 包名   
import 包名 as 别名   
...
**numpy**：Python科学计算基础软件包，用于处理N维数组、矩阵。   
